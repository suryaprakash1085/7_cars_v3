  ...
  useEffect(() => {
    ...
    const fetchWorkReport = async () => {
      try {
        ...
        // console.log("work report", data);
        setWorkReport(data);
      } catch (error) {
        console.log("Error fetching work report:", error);
      } finally {
        setLoading(false);
      }
    };
...
const pageType = Cookies.get("page_type"); // "tab" or others
  ...
  return (
    <div>
      ...
      {pageType !== "tab" && <Navbar pageName="Time Entry" /> }
      <Box sx={{ padding: "20px" }}>
        <div
          style={{
            display: "flex",
                        ...
                        <Box sx={{ minHeight: 40, p: 1 }}>
                          ...
                          {!employee.shifts.filter(
                            ...
                            <Button
                              ...
                              onClick={() => {
                                if (!workReport) {
                                  console.log("Work report is not loaded yet");
                                  return;
                                }
                                handleOpenShiftModal(
                                  employee,
