  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const company = response?.data?.company_details?.[0];
        ...
        setPdfLogo(company?.logo || "");
        setCompanyDetails(response?.data?.company_details || []);

        // console.log('pdfHeaderImage', company?.pdf_header)
        // console.log('pdfFooterImage', company?.pdf_header)
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      ...
      try {
        ...
        // Load next service km settings from database
        const savedNextServiceKm = appointmentData.next_service_km;
        const savedNextServiceKmEnabled =
          savedNextServiceKm !== null &&
          savedNextServiceKm !== undefined &&
          savedNextServiceKm !== 0;
...
if (appointmentData.sales_id) {
  ...
  // Fallback: use logged-in user from cookies
  const cookieUserId = Cookies.get("userId");
  if (cookieUserId) {
    setSelectedSales([cookieUserId]);
  }
  // If still no value, set display name from userName cookie
  // (this is just a visual fallback — actual ID is still needed for API)
        ...
        if (
          ...
          setReferredBy(appointmentData.referred_by.trim());
        } else if (
          customerData.reference &&
          customerData.reference.trim() !== ""
        ) {
          setReferredBy(customerData.reference.trim()); // "Customer reference"
        }

        //  Customer Reference Name — appointment first, then customer.referred_by fallback
        if (
          appointmentData.customer_ref_name &&
          appointmentData.customer_ref_name.trim() !== ""
        ) {
          setCustomerRefName(appointmentData.customer_ref_name.trim());
        } else if (
          customerData.referred_by &&
          customerData.referred_by.trim() !== ""
        ) {
          setCustomerRefName(customerData.referred_by.trim()); // "1234"
        }
          ...
          appointmentData.services_actual.forEach((service) => {
            ...
            if (service.items_required && service.items_required.length > 0) {
              service.items_required.forEach((item) => {
                // Check if this spare already exists to avoid duplicates
                const spareExists = servicesByDescription[
                  service.service_description
                ].spares.some(
                  (spare) =>
                    spare.spareList === (item.item_name || "") &&
                    spare.qty === (item.qty || 0),
                );
                if (!spareExists) {
  ...
  // Wait — if appointment already set sales, don't override
  if (selectedSales && selectedSales.length > 0) return;

  const cookieUserName = Cookies.get("userName"); // "suriya"
  const cookieUserId = Cookies.get("userId");     // "USER-400014"

  // Try match by userName first
  if (cookieUserName) {
    const matchedByName = allUsers.find(
      (u) =>
        `${u.firstName} ${u.lastName}`.toLowerCase().trim() ===
        cookieUserName.toLowerCase().trim(),
    );
    if (matchedByName) {
      setSelectedSales([matchedByName.user_id]);
      return;
    }
  }

  // Fallback: match by userId cookie
  if (cookieUserId) {
    const matchedById = allUsers.find(
      (u) => String(u.user_id) === String(cookieUserId),
    );
    if (matchedById) {
      setSelectedSales([matchedById.user_id]);
    }
  }
}, [allUsers, selectedSales]);

  // helper that removes blank spare rows and empty items
  const getCleanEstimateItems = () => {
    const cleaned = estimateItems
      .map((item) => ({
        ...item,
  ...
  const handleGeneratePDF = async () => {
    ...
    // remove blank rows before sending to generator
    const cleanedItems = getCleanEstimateItems();

    if (cleanedItems.length === 0) {
      setSnackbarMessage(
        "Cannot generate PDF because there are no valid spare items.",
      );
      setOpenSnackbar(true);
      setIsSaving(false);
      return;
    ...
    if (cleanedItems.length !== estimateItems.length) {
      setSnackbarMessage(
        "Blank spare rows have been removed for PDF generation.",
      );
      setOpenSnackbar(true);
    }

    // recompute totals based on cleanedItems
    ...
    const cleanedGrandTotal = computeGrandTotal(cleanedItems);
  ...
  const previewPDF = () => {
    ...
    if (cleanedItems.length === 0) {
      setSnackbarMessage(
        "Cannot preview PDF because there are no valid spare items.",
      );
      setOpenSnackbar(true);
      return;
    }
    if (cleanedItems.length !== estimateItems.length) {
      setSnackbarMessage("Blank spare rows will not be shown in preview.");
      setOpenSnackbar(true);
    }
...
console.log("formattedDate_appoinment_date", formattedDate_appoinment_date);
        // Set the selected mechanic from appointment data (already an array from backend)
        setSelectedMechanic(
          Array.isArray(appointmentData.mechanic_id)
  ...
  const handleSaveSalesInfo = async () => {
    ...
    if (!selectedSales || selectedSales.length === 0) {
      setSnackbarMessage("Please select a Sales person before saving");
      setSeverity("error");
      setOpenSnackbar(true);
      return false; // stop
    }
    ...
    if (!referredBy) {
      setSnackbarMessage("Please select Referred By before saving");
      setSeverity("error");
      setOpenSnackbar(true);
      return false; //  stop
    }
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/appointment/${appointmentId}/update_sales_info`,
        {
          ...
          body: JSON.stringify({
            sales_id: Array.isArray(selectedSales)
              ? selectedSales
              : [selectedSales],
  ...
  const updateSpareItem = (index, spareIndex, field, value) => {
    setEstimateItems((prevItems) => {
      const updatedItems = [...prevItems];
      // sanitize numeric fields to prevent negatives
      let newValue = value;
      if (["qty", "price"].includes(field)) {
    ...
    if (!is_empty && !is_zero) {
      try {
        ...
        try {
          ...
          console.warn(
            "Failed to fetch updated appointment for procurement:",
            err,
          );
        }
  ...
  const createProcurement = async (itemsToProcure) => {
    ...
    try {
      ...
      const itemsNeedingProcurement = uniqueItems.filter(
        (item) => item.shortage_qty > 0,
      );
  ...
  //!! Updating Next Service KM in the database
  ...
  useEffect(() => {
    if (!appointmentLoadedRef.current) {
      return;
    }
  ...
  const handleNextServiceKmChange = (event) => {
    ...
    // Save to database instead of cookies
    updateNextServiceKm(inputValue);
    setHasChanges(true);
  };

  const resetNextServiceKm = () => {
    setNextServiceKmManual(false);
    ...
    if (Number.isFinite(currentKm)) {
      ...
      setNextServiceKm(computed);
      updateNextServiceKm(computed);
      setSnackbarMessage("Next Service KM reset to auto value");
      setSeverity("success");
      setOpenSnackbar(true);
    }
  };
  ...
  const handleSaveSpareEdit = () => {
    ...
    setEstimateItems((prevItems) => {
      ...
      updatedItems[editingSpare.itemIndex].spares[0] = {
        ...
        qty: Math.max(0, parseFloat(editedSpareData.qty) || 0),
        price: Math.max(0, parseFloat(editedSpareData.price) || 0),
      };
  ...
  const assignMechanic = async (mechanicId, mechanicName) => {
    try {
      ...
      setSelectedMechanic(
        Array.isArray(mechanicId) ? mechanicId : [mechanicId],
      );
      setSeverity("success");
  ...
  const saveSalesInfo = async (salesIds, salesNames) => {
    try {
      // Only save if referredBy is also set (API requires both fields)
      if (!referredBy) {
        setSnackbarMessage("Please select 'Referred By' before saving Sales");
        setOpenSnackbar(true);
        return;
      }

      // Send all sales IDs as array - API now supports multiple sales persons
      const salesToSend = Array.isArray(salesIds) ? salesIds : [salesIds];
      ...
      // Update appointment data log with new sales_id
      setAppointmentDataLog((prev) => ({
        ...prev,
        sales_id: salesToSend,
      }));

      const namesText = Array.isArray(salesNames)
        ? salesNames.join(", ")
        : salesNames;
      setSeverity("success");
      setSnackbarMessage(`Sales ${namesText} assigned successfully`);
      setOpenSnackbar(true);
    } catch (error) {
      console.error("Error saving sales info:", error);
      setSnackbarMessage("Error saving sales info");
  ...
  const saveReferredByInfo = async (referredByValue) => {
    try {
      // Only save if sales are also selected (API requires both fields)
      if (!selectedSales || selectedSales.length === 0) {
        setSnackbarMessage("Please select 'Sales' before saving Referred By");
        setOpenSnackbar(true);
        return;
      }

      // Send all sales IDs as array - API now supports multiple sales persons
      const salesToSend = Array.isArray(selectedSales)
        ? selectedSales
        : [selectedSales];
      ...
      if (!response.ok) throw new Error("Failed to save referred by info");

      // Update appointment data log with new referred_by
      setAppointmentDataLog((prev) => ({
        ...prev,
        referred_by: referredByValue,
      }));

      setSnackbarMessage(`Referred By ${referredByValue} saved successfully`);
      setOpenSnackbar(true);
    } catch (error) {
      console.error("Error saving referred by info:", error);
      setSnackbarMessage("Error saving referred by info");
      setOpenSnackbar(true);
    }
  };

  const saveCustomerRefName = async (refName) => {
    try {
      // Only save if referredBy is "Customer reference" and sales are selected
      if (referredBy !== "Customer reference") {
        return;
      }

      if (!selectedSales || selectedSales.length === 0) {
        setSnackbarMessage(
          "Please select 'Sales' before saving Customer Reference Name",
        );
        setOpenSnackbar(true);
        return;
      }
      ...
      if (!response.ok)
        throw new Error("Failed to save customer reference name");

      // Update appointment data log with new customer_ref_name
      setAppointmentDataLog((prev) => ({
        ...prev,
        customer_ref_name: refName,
      }));
      ...
      setSeverity("success");
      setSnackbarMessage("Customer Reference Name saved successfully");
      setOpenSnackbar(true);
    } catch (error) {
      console.error("Error saving customer reference name:", error);
      setSnackbarMessage("Error saving customer reference name");
      setOpenSnackbar(true);
    }
  };

  const saveKilometer = async (newKm) => {
    if (newKm === "" || newKm === "0" || newKm === 0) {
      setSnackbarMessage("KM cannot be empty or 0");
      setSeverity("error");
      setOpenSnackbar(true);
      return;
    ...
    try {
      await updateKm(newKm);
    } catch (error) {
      console.error("Error saving kilometer:", error);
    }
  };
  ...
  const handleStatusUpdate = async () => {
    /* ---------------------------------------
       1️⃣ VALIDATE REPORTED ISSUE
    --------------------------------------- */
    const hasInvalidIssue = estimateItems.some(
      (item) => !item.reportedIssue || item.reportedIssue.trim() === "",
    );

    if (hasInvalidIssue) {
      setSnackbarMessage("Reported Issue cannot be empty or spaces only.");
      setSeverity("error");
      setOpenSnackbar(true);
      return;
    }
    ...
    /* ---------------------------------------
       2️⃣ VALIDATE AT LEAST ONE REAL SPARE
    --------------------------------------- */
    const hasValidSpare = estimateItems.some(
      (item) =>
        Array.isArray(item.spares) &&
        item.spares.some(
          (spare) => spare.spareList && spare.spareList.trim() !== "",
        ),
    );
    ...
    if (!hasValidSpare) {
      setSnackbarMessage(
        "Please add at least one spare part before invoicing.",
      );
      setSeverity("error");
      setOpenSnackbar(true);
      return;
    ...
    /* ---------------------------------------
       3️⃣ SAVE SERVICES AND SPARES FIRST
    --------------------------------------- */
    setIsSaving(true);

    try {
      // First save the estimate items (spares/services) to database
      await saveEstimateItems();

      /* ---------------------------------------
         4️⃣ UPDATE STATUS → INVOICE
      --------------------------------------- */
      const token = Cookies.get("token");
      ...
      setSnackbarMessage("Invoice status sent successfully");
      setSeverity("success");
      setOpenSnackbar(true);

      // Navigate to invoice list after showing success message
      setTimeout(() => {
  ...
  // Helper function to save estimate items (spares/services)
  const saveEstimateItems = async () => {
    const token = Cookies.get("token");
    const validItems = estimateItems.filter((item) => item.reportedIssue);
    ...
    // Check that each service has at least one spare item
    const servicesWithoutSpares = itemsWithFilledSpares.filter(
      (item) => item.spares.length === 0,
    );
    if (servicesWithoutSpares.length > 0) {
      throw new Error(
        `${servicesWithoutSpares.length} service(s) have no spare parts. Each service must have at least one spare part.`,
      );
    }
    ...
    const services = itemsWithFilledSpares.flatMap((item) => {
      return item.spares.map((spare) => ({
        service_id: spare.new != "X" ? spare.service_id : "",
        service_description: item.reportedIssue,
        price: item.estimatedAmount,
        service_type: item.type,
        items_required: [
    ...
    if (!response.ok) {
      throw new Error("Failed to save services and spares");
    }

    const responseData = await response.json();

    // After successful POST, fetch the updated appointment to get service_ids
    try {
      const apptResp = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/appointment/${appointmentId}`,
        {
      ...
      if (apptResp.ok) {
        ...
        // Create procurement if there are items to procure
        if (itemsToProcureFromResponse.length > 0) {
          await createProcurement(itemsToProcureFromResponse);
        }
      }
  ...
  return (
    <div>
      ...
      {pageType !== "tab" && (
        <Navbar pageName="Estimate Details" hasChanges={hasChanges} />
      )}
      <Dialog
            ...
            <Box display="flex" flexDirection="column" gap={3}>
              ...
              >
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: "220px", marginLeft: 16 }}>
                    ...
                    >
                      ...
                      <br />
                      {customer.contact.address.street},<br />
                ...
                >
                    ...
                    >
                      {users.map((user) => (
                        <MenuItem key={user.user_id} value={user.user_id}>
                          <Checkbox
                            checked={selectedMechanic.includes(user.user_id)}
                          />
                          <ListItemText
                  ...
                  <FormControl sx={{ width: 220 }} size="small">
                    ...
                    >
                      {allUsers.map((user) => (
                        <MenuItem key={user.user_id} value={user.user_id}>
                          <Checkbox
                            checked={selectedSales.includes(user.user_id)}
                          />
                          <ListItemText
                            primary={`${user.firstName} ${user.lastName}`}
                          />
                  ...
                  <FormControl sx={{ width: 200 }} size="small">
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        setReferredBy(selectedValue);
                        saveReferredByInfo(selectedValue);
                      }}
                    >
                      <MenuItem value="Online">Online</MenuItem>
                      <MenuItem value="Tele in">Tele in</MenuItem>
                      <MenuItem value="Telecalling out">
                        Telecalling out
                      </MenuItem>
                      <MenuItem value="Walk-in">Walk-in</MenuItem>
                      <MenuItem value="Customer reference">
                        Customer reference
                      </MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  {/* STATUS BUTTON */}
                  <Tooltip title="Released to invoice">
                    <Button
                      variant="contained"
                                                  ...
                                                  >
                                                    <Autocomplete
                                                      ...
                                                      filterOptions={(
                                                        ...
                                                        if (!inputValue)
                                                          return options;
                                                        // match only if option starts with typed letters
                                                  ...
                                                  <TableCell align="center">
                                                    <Button
                                                      ...
                                                      onClick={() => {
                                                        ...
                                                        // Check if this spare already exists in the list
                                                        const spareExists =
                                                          estimateItems[
                                                            index
                                                          ].spares.some(
                                                        ...
                                                        if (spareExists) {
                                                          setSeverity("error");
                                                          setSnackbarMessage(
                                                            "This spare part already exists",
                                                          );
                                                          setOpenSnackbar(true);
                                                          return;
                                                        }
                                                            ...
                                                            {isEditing ? (
                                                              <Autocomplete
                                                                ...
                                                                filterOptions={(
                                                                  ...
                                                                  // Only match options that start with the current input
                                                                  return opts.filter(
                                                                    (o) =>
                                                                      o
                                                                        .toLowerCase()
                                                              ...
                                                              >
                                                                <Tooltip title="Edit Spare">
                                                                  <IconButton
                                                                    size="small"
                                                                    onClick={() => {
                                                                      // use the actual unfiltered index computed above
                                                                      handleEditSpare(
                                                                        index,
                                                                        actualSpareIndex,
                                                                      );
                                                                ...
                                                                <Tooltip title="Delete Spare">
                                                                  <IconButton
                                                                    size="small"
                                                                    onClick={() => {
                                                                      // pass the actual index we calculated earlier
  ...
  try {
    // First save sales info
    const salesSaved = await handleSaveSalesInfo();
