...
function ExecutiveSummary({ startDate, endDate, companyDetails }) {
    ...
    // Add this line before return in ExecutiveSummary
//     console.log("Total appointments:", appointmentsList.length);
// console.log("Appointment statuses:", appointmentsList.map(a => a?.status));
// console.log("Service Efficiency:", serviceEfficiency);
// console.log("Total appointments:", appointmentsList.length);
// console.log("Unique customer IDs:", [...new Set(appointmentsList.map(a => a?.customer_id))]);
// console.log("Vehicle Base count:", customersList.length);
// console.log("Sample appointment:", appointmentsList[0]); 
console.log("Total appointments count:", appointmentsList.length);
console.log("Date range:", startDate, "to", endDate);
console.log("First appointment:", appointmentsList[0]);
console.log("Last appointment:", appointmentsList[appointmentsList.length - 1]);
    return (
        <Box
            sx={{
...
// console.log("ServiceAnalytics endDate:", endDate);

// console.log("ServiceAnalytics first appointment:", appointmentsList[0]);
// console.log("ServiceAnalytics last appointment:", appointmentsList[appointmentsList.length - 1]);
console.log("Total appointments:", appointmentsList.length);

// Completed
console.log("Completed (invoiced) count:", appointmentsList.filter(apt => apt?.status?.toLowerCase().includes('invoiced')).length);
console.log("Completed statuses:", appointmentsList.filter(apt => apt?.status?.toLowerCase().includes('invoiced')).map(apt => apt?.status));

// Pending
console.log("Pending (scheduled) count:", appointmentsList.filter(apt => apt?.status?.toLowerCase().includes('scheduled')).length);
console.log("Pending statuses:", appointmentsList.filter(apt => apt?.status?.toLowerCase().includes('scheduled')).map(apt => apt?.status));

// All statuses - to see what values exist
console.log("All unique statuses:", [...new Set(appointmentsList.map(apt => apt?.status))]);
    return (
        <Box
            sx={{
                backgroundImage: bgImage
...
// });
const customersList = (Array.isArray(customers) ? customers : []).filter(customer => {
    if (!startDate || !endDate) return true; // show all if no date
    const customerDate = new Date(customer?.created_at);
    const start = new Date(startDate);
    const end = new Date(endDate);
   ...
   const bgImage = companyDetails?.background_image || null;
   console.log("Total Customers:", customersList.length);
console.log("Sample customer:", customersList[0]);
console.log("New Customer dates:", customersList.map(c => c?.created_at).slice(0, 10));
console.log("Sample vehicles:", customersList.slice(0, 5).map(c => ({
    name: c?.customer_name,
    vehicleCount: c?.vehicles?.length,
    vehicles: c?.vehicles
})));
console.log("Fleet Size (total vehicles):", allVehicles.length);
...
// // Net Revenue
// console.log("Net Revenue (Credits - Debits):", netRevenue);

// // Inventory
// console.log("Inventory Items:", inventoryList);
// console.log("Total Inventory Value:", totalInventoryValue);

// // Sample check
// console.log("Sample Transaction:", transactionsList[0]);
// console.log("Sample Inventory Item:", inventoryList[0]);
    return (
        <Box
            sx={{
                backgroundImage: bgImage
...
//     console.log("------ OPERATIONS CENTER DEBUG ------");
...
// // Geo grouping
// console.log("Geo Data (city/state grouping):", geoData);
...
//   console.log("Service Rate:", serviceUtilization);
