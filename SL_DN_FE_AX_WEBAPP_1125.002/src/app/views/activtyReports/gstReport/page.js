...
export default function GSTReport() {
  ...
  const fetchGSTReports = async (start, end) => {
    ...
    try {
      ...
      // Fetch appointments data using the GST-specific endpoint
      const params = new URLSearchParams();
      if (start) params.append("startDate", start);
      if (end) params.append("endDate", end);
      params.append("status", "invoiced");
      params.append("gstFilter", "converted");
      ...
      // Map appointments directly (backend already filters for GST with gstFilter=converted)
      const gstData = appointments
        .map((apt) => {
