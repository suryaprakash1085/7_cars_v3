...
const CustomerDetail = () => {
          ...
          appointmentData.services_actual.forEach((service) => {
            if (!servicesByDescription[service.service_description]) {
              servicesByDescription[service.service_description] = {
                ...
                mechanicalDesignation: service.mechanical_designation || "",
                spares: [],
  ...
  const getSearchFilteredInventory = (searchValue) => {
    if (!searchValue || searchValue.trim() === "") {
      // return inventory.map((option) => option.part_name);
  ...
  const validateAndPostService = async (
        ...
        return [
          {
            ...
            mechanical_designation: item.mechanicalDesignation || "",
    ...
    if (!is_empty && !is_zero) {
      try {
        console.log("Check 4");
        // First POST the services so the backend can create service IDs
        const response = await fetch(
  ...
  const createProcurement = async (itemsToProcure) => {
    const token = Cookies.get("token");
    try {
      // Remove duplicates based on item_id and product combination
      const seenItems = new Set();
      const uniqueItems = itemsToProcure.filter((item) => {
        const key = `${item.item_id}-${item.product}`;
 ...
 const handleStatusUpdate = async () => {
  ...
  try {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/appointment/${appointmentId}/update_status`,
      {
        ...
        body: JSON.stringify({ status: "invoice" }), // only invoice
      }
    );
    ...
    setSeverity("success");
    setOpenSnackbar(true);
    setStatusChange(""); // reset dropdown
    router.push(`/views/invoiceList/${appointmentId}`);
  } catch (error) {
    console.error("Error sending invoice status:", error);
    setSnackbarMessage("Error sending invoice status");
    setSeverity("error");
    setOpenSnackbar(true);
  }
};
              ...
              >
                ...
                >
                  ...
                  >
                   ...
                   <Tooltip title="relsed to invoice">
  <Button
    variant="contained"
    color="primary"
    style={{ height: "38px" }}
  ...
  onChange={(e) => updateEstimateItem(index, "reportedIssue", e.target.value)}
  size="small"
  fullWidth
  multiline
  rows={2}
...
/>
                                                  ...
                                                  <TableCell align="center">
                                                    <Button
                                                      ...
                                                      onClick={() => {
                                                        ...
                                                        if (spareExists) {
                                                          setSnackbarMessage(
                                                            "This spare part already exists with the same quantity"
                                                          );
                                                          setOpenSnackbar(true);
                                                          return;
                                                        }
                    ...
                    <Box display="flex" justifyContent="flex-end" marginTop="10px" marginRight="10px">
  <IconButton
    onClick={() => addEstimateItem()}
    sx={{
      color: "green",
    ...
    title="Add New Row"
  >
    <AddIcon sx={{ fontSize: 28 }} /> {/* optional: bigger icon */}
  </IconButton>
</Box>
