...
import 'yet-another-react-lightbox/plugins/captions.css'; // Import the captions styles
...
export default function SubcategoryPage() {
  ...
  const [currentImageIndex, setCurrentImageIndex] = useState(0); // Current image index for lightbox
  ...
  useEffect(() => {
    ...
    const fetchSubcategories = async () => {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/servegallery/${subcategory}`, {
        headers: {
      ...
      const parsedData = data.map(subcategory => ({
        ...subcategory,
        images: JSON.parse(subcategory.image)
      }));
  ...
  const handleSubmit = async () => {
    ...
    formData.append("sub_id", subcategory);

    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/servegallery`, {
      method: "POST",
      body: formData,
      headers: {
        Authorization: `Bearer ${token}`,
...
const pageType = Cookies.get("page_type"); // "tab" or others
  return (
    <div>
      {/* <Navbar pageName="Gallery Images " /> */}
        {pageType !== "tab" && <Navbar pageName="Gallery Images" />}
      ...
      <Dialog open={openModal} onClose={() => setOpenModal(false)} sx={{ borderRadius: '10px', overflow: 'hidden' }}>
        <DialogTitle sx={{ backgroundColor: '#f5f5f5', fontWeight: 'bold', textAlign: 'center' }}>Add Image</DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '20px' }}>
          ...
          {/* <Button variant="outlined" onClick={handleAddRow} sx={{ marginTop: '10px' }}>Add Another Image</Button> */}
          <Button
      ...
      <Box display="flex" flexWrap="wrap" justifyContent="center">
        {subcategories.map((subcategory, index) => (
          <StyledCard key={subcategory.id} onClick={() => openImageLightbox(index)}>
            <CardMedia
              component="img"
              height="200"
              image={`${process.env.NEXT_PUBLIC_API_URL}/servegallery/image/${subcategory.images[0]}`} // Display the first image
              alt={subcategory.name}
              style={{ objectFit: 'cover' }} // Ensure images fit well
            />
            ...
            <Button 
              ...
              onClick={async (e) => {
                ...
                const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/servegallery/${subcategory.id}`, {
                  method: "DELETE",
                  headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                ...
                if (response.ok) {
                  // Optionally refresh subcategories or update state to remove the deleted item
                  setSubcategories(subcategories.filter(item => item.id !== subcategory.id));
                } else {
                  console.error("Failed to delete servegallery item");
                }
              }}
              sx={{ margin: '10px' }}
            >
      ...
      <Lightbox
        plugins={[Captions, Zoom]} // Removed Thumbnails
        open={openLightbox}
        close={closeImageLightbox}
        slides={subcategories.map((subcategory) => ({
          src: `${process.env.NEXT_PUBLIC_API_URL}/servegallery/image/${subcategory.images[0]}`,
          title: subcategory.name, // Caption for the current image
          // description: `Description for ${subcategory.name}`, // Optional description
        }))}
        currentIndex={currentImageIndex}
        onCurrentIndexChange={setCurrentImageIndex}
        render={{
          // Optional: Customize the lightbox rendering
          header: (props) => <div>{props.title}</div>, // Custom header
        }}
      />
    </div>
  );
